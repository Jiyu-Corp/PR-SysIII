...
export default function FinishParkingServiceTab({ parkingService, closeTab, closeModal }: FinishParkingServiceTabProps) {
  ...
  async function handleFinishParkingService() {
    const params = {
      ...
      additionalDiscount: additionalDiscount && additionalDiscount != '' ? Number(additionalDiscount) : undefined
    }
    try {
  ...
  return <div className="finish-parking-tab">
    ...
    <div className="finish-parking-footer">
      <ButtonModal icon={ArrowUpRightIcon} text="Saída" action={handleFinishParkingService} color="#FFFFFF" backgroundColor="#3BB373" fontSize={12} isDisabled={(priceTotal == 0 && prices.length < 2) || typeof priceTotal !== 'undefined' && priceTotal < 0}/>
    </div>
  </div>
}